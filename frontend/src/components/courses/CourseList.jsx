...
const CourseList = () => {
  ...
  useEffect(() => {
    const fetchAllCourses = async () => {
      ...
      try {
        ...
        const allFetchedCourses = Array.isArray(res.data.courses)
          ? res.data.courses
          : res.data;

        const activeData = allFetchedCourses.filter((c) => c.status === "active");
  ...
  useEffect(() => {
    ...
    // Category
    if (filter.category.length) {
      filteredData = filteredData.filter((c) => filter.category.includes(c.category));
    }

    // Instructor
    if (filter.instructor.length) {
      filteredData = filteredData.filter((c) =>
        filter.instructor.includes(c.instructor)
      );
    }
    ...
    if (filter.price !== "All") {
      filteredData = filteredData.filter((c) => {
        if (filter.price === "Free" && c.price === 0) return true;
        if (filter.price === "Paid" && c.price > 0) return true;
        return false;
      });
    }
    ...
    // Review
    if (filter.review > 0) {
      filteredData = filteredData.filter((c) => c.rating >= filter.review);
    }

    // Level
    if (filter.level !== "All levels") {
      filteredData = filteredData.filter((c) => c.level === filter.level);
    }
