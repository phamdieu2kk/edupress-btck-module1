...
//   // Tính tổng USD từ giỏ hàng
//   const totalUSD = selectedItems.reduce((sum, i) => sum + i.course.price * i.quantity, 0);

//   // Tổng VND
//   const totalVND = totalUSD * 5000;
...
//       orderId: currentOrderId,
//       cart: selectedItems,
//       total: totalVND, // VND nguyên
//     });

//     const backendOrderId = orderRes?.data?.order?.orderId;
//     if (!backendOrderId) throw new Error("OrderId backend trả về undefined");

//     // 2️⃣ Gọi backend tạo URL VNPay
//     const vnpRes = await axiosClient.post("/vnpay/create-payment", {
//       orderId: backendOrderId,
//       amount: totalVND, // VND nguyên, BE nhân 100
...
//     // 3️⃣ Redirect sang VNPay sandbox
//     window.location.href = paymentUrl;
...
//               <Typography fontWeight={700} color="error">{selectedItems.length ? formatCurrencyDisplay(totalUSD) : "0₫"}</Typography>
//             </Stack>
...
const Payment = () => {
  ...
  // Tổng VND từ cart
  const totalVND = selectedItems.reduce((sum, i) => sum + i.course.price * i.quantity, 0);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const orderRes = await axiosClient.post("/orders/create", {
        orderId: currentOrderId,
        cart: selectedItems,
        total: totalVND, // lưu backend VND thật
      });
      const backendOrderId = orderRes?.data?.order?.orderId;
      if (!backendOrderId) throw new Error("OrderId backend trả về undefined");
      ...
      const vnpRes = await axiosClient.post("/vnpay/create-payment", {
        orderId: backendOrderId,
        amount: formatCurrencyPayment(totalVND), // VNPay chuẩn số nguyên VND
        bankCode: selectedBank || undefined,
      });
          ...
          <Paper sx={{ p: 3, borderRadius: 2, bgcolor: "#fff" }}>
            ...
            {selectedItems.map((item, idx) => (
              <Stack key={idx} direction="row" justifyContent="space-between" mb={1}>
                <Typography sx={{ maxWidth: "70%", wordWrap: "break-word" }}>{item.course.title} × {item.quantity}</Typography>
                <Typography sx={{ flexShrink: 0 }}>{formatCurrencyDisplay(item.course.price * item.quantity)}</Typography>
              </Stack>
            ))}
            <Divider sx={{ my: 2 }} />
            <Stack direction="row" justifyContent="space-between" mt={2}>
...
//   const handlePayment = () => {
//     if (!agree || !selectedItems.length) return;

//     // Redirect trực tiếp sang PaymentResult
...
//             <Stack direction="row" justifyContent="space-between" mt={2}>
//               <Typography fontWeight={700}>Tổng</Typography>
//               <Typography fontWeight={700} color="error">{formatCurrencyDisplay(totalUSD)}</Typography>
//             </Stack>
