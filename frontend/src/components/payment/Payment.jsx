...
// import axiosClient from "../../api/axiosClient"; // FE axios
// import { formatCurrencyDisplay, formatCurrencyPayment } from "../../utils/helpers";
...
//     const backendOrderId = orderRes?.orderId || orderRes?.data?.orderId;
//     if (!backendOrderId) throw new Error("OrderId backend trả về undefined");

//     // 2. GỌI BACKEND TẠO URL VNPAY (ĐÃ SỬA)
//     // Thay đổi từ GET sang POST
//     const vnpRes = await axiosClient.post("/vnpay/create-payment", {
//       // Truyền dữ liệu vào body của request
//       amount: formatCurrencyPayment(total),
//       orderId: backendOrderId,
//       bankCode: selectedBank,
//     });

//     const paymentUrl = vnpRes?.paymentUrl || vnpRes?.data?.paymentUrl;
//     if (!paymentUrl) throw new Error("VNPay trả về undefined paymentUrl");
...
//               <Typography>{selectedItems.length ? formatCurrencyDisplay(0) : "-"}</Typography>
//             </Stack>
//             <Stack direction="row" justifyContent="space-between" mb={1}>
...
//               <Typography fontWeight={700} color="error">{selectedItems.length ? formatCurrencyDisplay(total) : "0₫"}</Typography>
//             </Stack>
...
// const banks = [
//   { id: "NCB", name: "Ngân hàng NCB", logo: "https://sandbox.vnpayment.vn/paymentv2/images/brands/NCB.svg" },
//   { id: "BIDV", name: "Ngân hàng BIDV", logo: "https://sandbox.vnpayment.vn/paymentv2/images/brands/BIDV.svg" },
//   { id: "VIETCOMBANK", name: "Ngân hàng Vietcombank", logo: "https://sandbox.vnpayment.vn/paymentv2/images/brands/VIETCOMBANK.svg" },
//   { id: "VIETINBANK", name: "Ngân hàng Vietinbank", logo: "https://sandbox.vnpayment.vn/paymentv2/images/brands/VIETINBANK.svg" },
//   { id: "AGRIBANK", name: "Ngân hàng Agribank", logo: "https://sandbox.vnpayment.vn/paymentv2/images/brands/AGRIBANK.svg" },
//   { id: "SACOMBANK", name: "Ngân hàng Sacombank", logo: "https://sandbox.vnpayment.vn/paymentv2/images/brands/SACOMBANK.svg" },
//   { id: "TECHCOMBANK", name: "Ngân hàng Techcombank", logo: "https://sandbox.vnpayment.vn/paymentv2/images/brands/TECHCOMBANK.svg" },
// ];
...
//                     </Stack>
//                   }
//                   sx={{ mb: 2, border: method === pm.id ? "2px solid #1976d2" : "1px solid solid #e0e0e0", borderRadius: 1, p: 1.5 }}
//                 />
//               ))}
//             </RadioGroup>

//             <Collapse in={method === "vnpay"}>
//               <Box mt={2}>
//                 <Typography fontWeight={600} mb={1}>Chọn ngân hàng</Typography>
//                 <Select
//                   fullWidth
...
const Payment = () => {
  ...
  const handlePayment = async () => {
    ...
    try {
      const vnpRes = await axiosClient.post("/vnpay/create-payment", {
        amount: total, // gửi VND trực tiếp
        orderId: currentOrderId,
        bankCode: selectedBank,
      });

      const paymentUrl = vnpRes?.data?.vnpUrl; // sửa key theo backend
      if (!paymentUrl) throw new Error("VNPay trả về undefined paymentUrl");
      ...
      <Box display="flex" flexDirection={{ xs: "column", md: "row" }} gap={3}>
          ...
          <Paper sx={{ p: 3, mb: 3, borderRadius: 2, boxShadow: "0 4px 12px rgba(0,0,0,0.05)", bgcolor: "#fff" }}>
            ...
            <Collapse in={method === "vnpay"}>
              <Box mt={2}>
                ...
                >
                  <MenuItem value="">
                    <Typography color="text.secondary">-- Chọn ngân hàng --</Typography>
                  </MenuItem>
                  {banks.map((b) => (
                    <MenuItem key={b.id} value={b.id}>
                      <Stack direction="row" spacing={1.5} alignItems="center">
        ...
        <Box flex={{ xs: 1, md: 4 }}>
          <Paper sx={{ p: 3, mb: 3, borderRadius: 2, boxShadow: "0 4px 12px rgba(0,0,0,0.05)", bgcolor: "#fff" }}>
            ...
            <Stack direction="row" justifyContent="space-between" mt={2}>
              <Typography fontWeight={700}>Tổng</Typography>
              <Typography fontWeight={700} color="error">{formatCurrencyDisplay(total)}</Typography>
            </Stack>
          </Paper>
        </Box>
      </Box>
