...
// ✅ Helpers hiển thị số nguyên
export const formatCurrencyDisplay = (amount) => {
  if (!amount || isNaN(amount) || amount <= 0) return "0";
  return Math.round(amount).toString(); // chỉ trả về số nguyên
};

export const formatCurrencyPayment = (amount) => {
  if (!amount || isNaN(amount) || amount <= 0) return 100;
      ...
      <Box display="flex" flexDirection={{ xs: "column", md: "row" }} gap={3}>
        ...
        <Box flex={{ xs: 1, md: 4 }}>
          <Paper sx={{ p: 3, mb: 3, borderRadius: 2, boxShadow: "0 4px 12px rgba(0,0,0,0.05)", bgcolor: "#fff" }}>
            ...
            <Stack direction="row" justifyContent="space-between" mb={1}>
              <Typography>Phí:</Typography>
              <Typography>{selectedItems.length ? formatCurrencyDisplay(0) : "0"}</Typography>
            </Stack>
            <Stack direction="row" justifyContent="space-between" mb={1}>
              <Typography>Thuế:</Typography>
              <Typography>{selectedItems.length ? formatCurrencyDisplay(0) : "0"}</Typography>
            ...
            <Stack direction="row" justifyContent="space-between" mt={2}>
              <Typography fontWeight={700}>Tổng</Typography>
              <Typography fontWeight={700} color="error">{selectedItems.length ? formatCurrencyDisplay(totalVND) : "0"}</Typography>
            </Stack>
