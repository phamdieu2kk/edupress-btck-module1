...
// ✅ BASE_URL linh hoạt cho local + deploy
const BASE_URL = process.env.REACT_APP_API_URL || "http://localhost:5000/api/courses";
...
const Courses = () => {
  ...
  const fetchCourses = async (page = 0, limit = pageSize) => {
    try {
      setLoading(true);
      const res = await axios.get(`${BASE_URL}?page=${page + 1}&limit=${limit}`);
      setCourses(res.data.courses || []);
  ...
  const handleDeleteSelected = async () => {
    ...
    if (window.confirm(`Are you sure you want to delete ${selectedCourses.length} courses?`)) {
      try {
        await Promise.all(selectedCourses.map(id => axios.delete(`${BASE_URL}/${id}`)));
        setSelectedCourses([]);
        fetchCourses(page, pageSize);
      } catch (err) {
        console.error("❌ Error deleting multiple courses:", err);
    ...
    <Box>
      ...
      {loading ? (
        ...
        <>
          ...
          {/* ... giữ nguyên phần render table như cũ ... */}
          {/* Chỉ cần thay axios URLs bằng BASE_URL */}
          <TableContainer component={Paper} sx={{ overflowX: "auto" }}>
            <Table sx={{ minWidth: 650 }}>
              <TableHead sx={{ bgcolor: "#FFF3E0" }}>
                {/* ... table headers ... */}
